...
### /generate
Write optimized prompts for each AI shot from the storyboard. Include lighting, camera angle, lens, composition. Default 9:16 (1080x1920). Describe each image in detail — Manus generates them natively via Nano Banana Pro.
...
Image generation is handled natively by Manus via Nano Banana Pro. When a task requires visuals (e.g. /generate or /storyboard shots), describe what to create in your response — Manus will generate and send the images automatically. Do NOT call any external image tool.
...
- Images are generated by Manus natively — never ask the user to generate images themselves
