...
program
  .name('claw-connector')
  .description('Bridge a local AI agent to the SLUGS mobile app via Supabase')
  .version('0.1.0');

program
  .command('connect')
  .description('Connect this agent to the SLUGS mobile app')
  .requiredOption('--token <token>', 'Your user ID token from the SLUGS app')
...
program
  .command('agent')
  .description('Run a Claude or OpenAI-powered AI agent connected to the SLUGS app')
  .requiredOption('--token <token>', 'Your user ID token from the SLUGS app')
  .option('--name <name>', 'A display name for this agent', 'Claude')
  .option('--system <prompt>', 'System prompt for the agent', 'You are a helpful AI assistant connected to the SLUGS app.')
