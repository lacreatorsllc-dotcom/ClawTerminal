...
async function writeReply(agentId: string, content: string) {
  await db.collection('agents').doc(agentId).collection('messages').add({
    agent_id: agentId,
    user_id: agentId,
    direction: 'outbound',
    content,
...
export function startChatListener(agentId: string) {
  console.log(`[blue-chip] starting listener for agent: ${agentId}`)
  const startTimestamp = Timestamp.now()
  const processed = new Set<string>()
  const MESSAGES_COL = db.collection('agents').doc(agentId).collection('messages')
